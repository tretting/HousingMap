...
# -------------------------------
gdf = gpd.read_file("data/Parcels/Updated_parcels_permits_with_preds_kfold1.geojson")
...
if enforce_zoning_areas:
    ...
    gdf.loc[~gdf['ZONING'].apply(zoning_allowed), 'predicted_prob_lg'] = np.nan
...
gdf['marker_size'] = gdf['predicted_prob_lg'].apply(lambda x: 5 if pd.notnull(x) else 0)
# Create hover text with four-digit rounding or leave blank if no data.
gdf['hover_text_lg'] = gdf['predicted_prob_lg'].apply(
...
score_min = 0
score_max = np.max(gdf["predicted_prob_lg"])
...
fig.add_trace(go.Scattermapbox(
    ...
    marker = dict(
        ...
        color = gdf["predicted_prob_lg"],
        colorscale = "Viridis",
        cmin = score_min,
        cmax = score_max,
        colorbar = dict(title = "Infill Score\n (logistic)")
    ),
    text = gdf["hover_text_lg"],
    hoverinfo = "text"
))
...
# 6. Write it out
minimal.to_file("data/Parcels/preds.geojson", driver="GeoJSON")
